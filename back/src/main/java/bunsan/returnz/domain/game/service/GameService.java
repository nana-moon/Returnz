...
public class GameService {
	...
	public HashMap<String, Object> getTurnInformation(String roomId, Long gamerId, boolean isCaptain) {
		...
		// 주식 평가 정보 update
		updateStockInformation(gameGamerStockDtos, roomId, gameRoomDto, gamerId);
		...
		if (isCaptain && !updateTurnInformation(gameGamerStockDtos, roomId, gameRoomDto, gamerId)) {
	...
	List<String> getBusinessDay(List<GameStockDto> gameStockDtoList, GameRoomDto gameRoomDto) {
		...
		for (GameStockDto gameStockDto : gameStockDtoList) {
			GameHistoricalPriceDayDto gameHistoricalPriceDayDto = gameHistoricalPriceDayService.findByDateTimeAndCompanyCode(
				gameRoomDto.getCurDate(), gameStockDto.getCompanyCode());
	...
	public HashMap<String, List<GameStockPriceInformationDto>> getStockPriceInformation(LocalDateTime curDate,
		...
		for (int i = 0; i < gameStockDtoList.size(); ++i) {
			...
			List<GameHistoricalPriceDayDto> gameHistoricalPriceDayDtos = gameHistoricalPriceDayService.findAllByDateTimeIsBeforeWithCodeLimit6(
				curDate, companyCode);
			...
			if (gameHistoricalPriceDayDtos.size() != 6) {
				...
				for (int x = gameHistoricalPriceDayDtos.size() - 2; x >= 0; --x) {
					...
					GameStockPriceInformationDto gameStockPriceInformationDto = GameStockPriceInformationDto.builder()
						...
						.historyDiff(Double.parseDouble(
							String.format("%.2f", Double.parseDouble(gameHistoricalPriceDayDtos.get(x).getClose())))
							- Double.parseDouble(String.format("%.2f",
							Double.parseDouble(gameHistoricalPriceDayDtos.get(x + 1).getClose()))))
						.historyUpAndDown(Double.parseDouble(
							String.format("%.2f", Double.parseDouble(gameHistoricalPriceDayDtos.get(x).getClose())))
							- Double.parseDouble(
							String.format("%.2f", Double.parseDouble(gameHistoricalPriceDayDtos.get(x + 1).getClose())))
							> 0 ? (Double.parseDouble(
							String.format("%.2f", Double.parseDouble(gameHistoricalPriceDayDtos.get(x).getClose())))
							- Double.parseDouble(
							String.format("%.2f", Double.parseDouble(gameHistoricalPriceDayDtos.get(x + 1).getClose())))
							== 0 ? StockState.STAY : StockState.UP) : StockState.DOWN)
				...
				mapGameStockPriceInformationDto.put(companyCode, gameStockPriceInformationDtos);
			}
		}
		// map 반환
	...
	public boolean updateStockInformation(List<GameGamerStockDto> gameGamerStockDtos, String roomId,
		...
		for (GameGamerStockDto gameGamerStockDto : gameGamerStockDtoList) {
			...
			if (gameRoomDto.getTurnPerTime().equals(TurnPerTime.DAY)) {
				stockPriceData = gameHistoricalPriceDayService.findByDateTimeIsAfterWithCodeLimit1(preDate,
					gameGamerStockDto.getCompanyCode());
			} else if (gameRoomDto.getTurnPerTime().equals(TurnPerTime.WEEK)) {
				stockPriceData = gameHistoricalPriceDayService.findByDateTimeAndCompanyCode(curDate,
					gameGamerStockDto.getCompanyCode());
			}
			...
			if (stockPriceData != null && stockPriceData.getMarket().equals("nasdaq")) {
				...
				stockClosePrice = Double.parseDouble(String.format("%.2f",
	...
	public boolean updateTurnInformation(List<GameGamerStockDto> gameGamerStockDtos, String roomId,
		...
		GameHistoricalPriceDayDto gameHistoricalPriceDayDtoBefore = gameHistoricalPriceDayService.findAllByDateTimeIsBeforeWithCodeLimit1(
	...
	public HashMap<String, List<GameHistoricalPriceDayDto>> getStockPriceWeek(List<GameStockDto> gameStockDtoList,
		...
		if (gameRoomDto.getCurTurn() == 0) {
			...
			for (int i = 0; i < gameStockDtoList.size(); ++i) {
				...
				List<GameHistoricalPriceDayDto> gameHistoricalPriceDayDtos = gameHistoricalPriceDayService.findAllByDateTimeIsBetweenWithCode(
	...
	public HashMap<String, Object> buyStock(GameBuySellRequestBody gameBuySellRequestBody) {
		...
		GameRoomDto gameRoomDto = gameRoomService.findByRoomId(roomId);
		GameHistoricalPriceDayDto gameHistoricalPriceDayDto = gameHistoricalPriceDayService.findByDateTimeAndCompanyCode(
			gameRoomDto.getPreDate(), companyCode);
		...
		if (gameGamerDto.getDeposit() >= (stockClosePrice * count)) {
			...
			PurchaseSaleLogDto purcahseSaleLogDto = PurchaseSaleLogDto.builder()
				...
				.companyName(gameCompanyDetailService.findByCompanyCode(companyCode).getKoName())
